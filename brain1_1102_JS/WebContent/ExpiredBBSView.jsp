    ...
    <div style="min-width:1200px;">
        ...
        <table class="board">
            <tr>
                ...
                <th width="200px">마감 기한</th>
            </tr>
            <c:choose>
            	<c:when test="${empty expiredBBSList}">
            		<tr>
            			<td colspan="3">만료된 계획이 없습니다.</td>
            		</tr>
            	</c:when>
            	<c:otherwise>
					<c:forEach var="item" items="${expiredBBSList}">
						<tr>
							<td>${item.seqNo}</td>
						<td><a href='./expiredBBSContent?seqNo=${item.seqNo}'>${item.title}</a></td>
						<td>${item.date}</td>
						</tr>
					</c:forEach>
				</c:otherwise>
			...
			<tr>
				<td align="center" colspan="3">
        			<c:if test="${pageNumber ne '1'}">
        				<a href="./expiredBBSView?pageNumber=${pageNumber-1}">PREV</a>
        			</c:if>
        			&nbsp;
        			<c:if test="${nextPage}">
        				<a href="./expiredBBSView?pageNumber=${pageNumber+1}">NEXT</a>
        			</c:if>
				</td>
			</tr>
        </table>
